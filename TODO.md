...
> ВЫПОЛНЕНО (2026-09-14). Три оформления, цветовой режим, Liquid Glass и быстрый
> призрак реализованы по `DESIGN_THEMES_AND_GHOST_TASK.md`. Итог — в
> `docs/design-integration-report.md`. Ниже — исходный скелет задачи (закрыт).

- [x] Прочитать файлы-инструкции по дизайну от владельца.
- [x] Подключить выбор темы: `ThemeProvider` читает `themeMode` + `designTheme`.
- [x] Добавить в Настройки выбор темы оформления из нескольких стандартных тем.
- [x] Выбор темы персистится (persist в settingsStore + безопасная гидратация).
- [x] Применить улучшения дизайна по инструкциям (токены, компоненты, экраны).
- [x] Проверки: `npm run typecheck`, `npm run lint`, `npm test`, сборка.
...
## ПРОДУКТОВАЯ ИТЕРАЦИЯ UI (redesign)

> ВЫПОЛНЕНО (2026-09-15). Задание — `AI_CODER_PRODUCT_UI_REDESIGN_TASK.md`.
> Итог: три оформления, единое нижнее меню с камерой/затвором, ghost reference,
> экран проекта-история, заметки/избранное/скрытие, compare на 3 режима.
> Подробности — в записи PROGRESS.md (2026-09-15) и DECISIONS.md.

- [x] Свести оформления к трём: Современный / Простой / Неоморфизм (+ миграция).
- [x] Modern платформенно-адаптивный (iOS glass / Android Material).
- [x] Единое нижнее меню с центральной кнопкой камеры → затвора.
- [x] Ghost reference: latest / first / manual per project + отметка эталона.
- [x] Убрать режим «90%», tap по превью временно усиливает призрак.
- [x] Экран проекта как история (summary, первое/последнее, quick compare, timeline).
- [x] Заметки, избранное, скрытие фото без удаления файла.
- [x] Flow A/B/C выбора пары и compare на 3 режима без экспорта файла.

### VERIFICATION (REDESIGN)

- [x] `npm run typecheck`, `npm run lint`, `npm test` (145/145), `npx expo-doctor` (21/21).
- [x] Бандл iOS собран и запущен без redbox/ошибок.
- [ ] Визуальная проверка тем/анимации/жестов — ручная (модель не читает скриншоты).
