...
- [x] 13.1 Установить expo-media-library:

```bash
npx expo install expo-media-library
```

- [x] 13.2 Реализовать permission flow для media library.
- [x] 13.3 Реализовать экспорт выбранного фото в галерею
       только по кнопке пользователя.
- [x] 13.4 Добавить понятное предупреждение, что после экспорта фото
       находится вне sandbox приложения.

### VERIFICATION 13

- Экспорт происходит только по явному действию пользователя. (кнопка «Экспорт»
  в просмотрщике + Alert-подтверждение с предупреждением)
- Без permission экспорт не выполняется. (requestMediaLibraryPermission до
  Asset.create; при отказе — Alert «Нет доступа к галерее»)
- Внутреннее хранение по-прежнему sandbox-first. (экспорт — копия в галерею,
  исходник остаётся в sandbox; см. DECISIONS.md)
- Запись в PROGRESS.md добавлена.
...
- [x] 14.1 Исследовать современные варианты генерации video/timelapse
       на текущем Expo/RN без deprecated библиотек.
- [x] 14.2 Проверить, не deprecated ли найденные решения.
- [x] 14.3 Проверить, требуют ли они custom native code.
- [x] 14.4 Записать варианты и риски в DECISIONS.md.
- [x] 14.5 Остановиться и спросить пользователя:
       реализовывать эту фичу или оставить вне MVP.
       (Решение: реализовать «псевдо-timelapse» без видеофайла; нативный модуль
       — факультативная задача на будущее, см. ниже)

### VERIFICATION 14

- Есть research note в DECISIONS.md. (запись от 2026-08-24)
- Нет кода timelapse до согласования. (реального видео-кодека нет)
- Пользователь принял решение и оно зафиксировано. (псевдо-timelapse)
- Запись в PROGRESS.md добавлена.
...
## ПОДГОТОВКА К ПУБЛИКАЦИИ — App icon и splash

> Сейчас иконка и splash — дефолтные Expo-заглушки. Полное руководство
...
## ДИЗАЙН И ТЕМЫ (следующая фаза)

> Рекомендации по улучшению дизайна владелец передаст отдельными
> файлами-инструкциями вместе со следующим промптом — прочитать их ПЕРВЫМИ.
> Этот раздел — скелет задачи; конкретика оформления — в тех файлах.

Контекст для старта:

- `AppSettings.themeMode` (`system | light | dark`) УЖЕ объявлен в
  `src/models/settings.ts` и есть в `DEFAULT_SETTINGS` (`src/store/settingsStore.ts`),
  но НЕ подключён к `ThemeProvider` — тот всё ещё следует только системной схеме
  (`useColorScheme` в `src/theme/ThemeProvider.tsx`). Это главная недоделка.
- Палитра задана в `src/theme/index.ts` (`lightColors` / `darkColors` + `ThemeColors`).

- [ ] Прочитать файлы-инструкции по дизайну от владельца.
- [ ] Подключить выбор темы: `ThemeProvider` должен читать `themeMode` из
      `settingsStore` и отдавать соответствующую палитру (system/light/dark).
- [ ] Добавить в Настройки UI выбора темы оформления из нескольких стандартных
      тем (расширить `ThemeMode`/палитры, если нужно больше, чем light/dark).
- [ ] Выбор темы персистится (уже есть persist в settingsStore — проверить).
- [ ] Применить улучшения дизайна по инструкциям владельца (токены, компоненты, экраны).
- [ ] Проверки: `npm run typecheck`, `npm run lint`, `npm test`, сборка.

### VERIFICATION (ДИЗАЙН И ТЕМЫ)

- Выбор темы меняет оформление сразу и переживает перезапуск (persist).
- Все экраны корректно отображаются в каждой теме.
- По возможности нет «магических» hex-цветов вне `src/theme`.
- Запись в PROGRESS.md добавлена.

---

## ФАКУЛЬТАТИВНО (на будущее) — Нативный модуль timelapse (генерация видео)

> Это НЕ обязательная задача MVP. Реализуется только по отдельному решению
> владельца продукта. Сейчас в приложении есть «псевдо-timelapse» (слайд-шоу
> по фото в UI без видеофайла) — см. DECISIONS.md.

Цель: генерировать настоящий видеофайл (mp4/mov) из последовательности фото
прогресса, локально на устройстве.

Выводы анализа (Фаза 14, 2026-08-24):

- Поддерживаемой НЕ-deprecated JS-библиотеки для склейки фото в видео в текущем
  стеке (Expo SDK 57 / RN 0.86) НЕТ.
- `ffmpeg-kit-react-native` и `react-native-ffmpeg` — deprecated/заброшены.
- `expo-video` — только воспроизведение, склейки изображений нет.
- Единственный «чистый» on-device путь — собственный нативный Expo Module:
  - iOS: `AVAssetWriter` + `AVAssetWriterInputPixelBufferAdaptor`;
  - Android: `MediaMuxer` + `MediaCodec` (или AndroidX Media3 Transformer,
    поддерживающий статичные изображения).

Чем полезен ИИ при реализации в будущем:

- Сгенерировать каркас нативного модуля (Swift/Kotlin) и JS-обёртку.
- Проверить совместимость API с актуальным SDK (читать типы node_modules).
- Написать unit-тесты для чистой логики (порядок кадров, тайминги) и мок
  нативного модуля для тестов.

Примерный план (когда решим делать):

- [ ] Создать local Expo Module (`npx create-expo-module --local`) с функцией
      генерации видео из массива file:// uri.
- [ ] iOS: AVAssetWriter с фиксированным размером/ориентацией кадров.
- [ ] Android: MediaMuxer + MediaCodec (кодирование H.264).
- [ ] JS-обёртка в /src/storage + тесты с моком.
- [ ] UI: кнопка «Создать видео» + экспорт результата (Фаза 13 уже умеет
      сохранять в галерею).

Риски: ручной нативный код, матрица устройств, обновление при смене SDK.
