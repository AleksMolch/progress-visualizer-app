...
```text
/src/app                  <- Expo Router root (в SDK 57 корень роутера внутри /src)
  _layout.tsx
  /(tabs)
    _layout.tsx
    index.tsx
...
Примечание (SDK 57): шаблон Expo кладёт корень Expo Router в `/src/app`,
а не в корневой `/app`, и использует alias `@/* -> ./src/*`. Это отклонение
от изначального черновика зафиксировано в DECISIONS.md.
